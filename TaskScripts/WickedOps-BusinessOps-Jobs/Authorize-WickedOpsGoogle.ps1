...
foreach($site in $cfg.Sites){$host=([uri]$site.Url).Host;$candidate=@($sites.siteEntry|Where-Object{$_.siteUrl -eq "sc-domain:$host" -or $_.siteUrl -eq $site.Url}|Select-Object -First 1);if($candidate){if($site.PSObject.Properties['SearchConsoleProperty']){$site.SearchConsoleProperty=$candidate.siteUrl}else{$site|Add-Member NoteProperty SearchConsoleProperty $candidate.siteUrl}}}
