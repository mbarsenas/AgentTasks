...
$counts=[ordered]@{PaidSessions=([regex]::Matches($text,'completed paid checkout session\(s\)').Count);Errors=([regex]::Matches($text,'\[ERROR\]').Count);Warnings=([regex]::Matches($text,'\[WARN\]').Count);BounceSignals=([regex]::Matches($text,'(?i)bounce|delivery failure').Count);ReplySignals=([regex]::Matches($text,'(?i)human repl|prospect repl').Count)}
