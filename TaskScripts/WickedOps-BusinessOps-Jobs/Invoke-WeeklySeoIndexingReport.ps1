...
$out=@()
foreach($site in $cfg.Sites){$property=if($site.PSObject.Properties['SearchConsoleProperty']){$site.SearchConsoleProperty}else{$site.Url};$uri='https://searchconsole.googleapis.com/webmasters/v3/sites/'+[uri]::EscapeDataString($property)+'/searchAnalytics/query';$body=@{startDate=$start.ToString('yyyy-MM-dd');endDate=$end.ToString('yyyy-MM-dd');dimensions=@('query');rowLimit=10};$r=Invoke-GoogleApi $t $uri Post $body;$clicks=(@($r.rows)|Measure-Object clicks -Sum).Sum;$impressions=(@($r.rows)|Measure-Object impressions -Sum).Sum;Write-WickedOpsLog $LogPath "$($site.Name): clicks=$([int]$clicks); impressions=$([int]$impressions); topQueries=$(@($r.rows).Count).";$out+=@{Product=$site.Name;Clicks=$clicks;Impressions=$impressions;Rows=@($r.rows)}}
